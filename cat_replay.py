...
class Shallow_Q_Learner(object):
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            y_target[0][action] = reward if done else reward + self.gamma * np.max(self.Q.predict(next_state)[0])
            x_batch.append(state[0])
            y_batch.append(y_target[0])
            
        self.Q.fit(np.array(x_batch), np.array(y_batch), batch_size=len(x_batch), verbose=0)
